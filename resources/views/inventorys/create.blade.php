                                                            ...
                                                            <div class="form-group">
                                                                ...
                                                                @if($myDepartments->count() > 0)
                                                                    <select class="form-control select2-dept" id="department2" name="rec_organize" style="width: 100%;" required>
                                                                        ...
                                                                        @foreach($myDepartments as $dep)
                                                                            ...
                                                                            @endphp
                                                                            <option value="{{ $dep->id }}" data-address="{{ $addressText }}">{{ $label }}</option>
                                                                        @endforeach
                                                                    </select>
                                                                    <input type="hidden" id="rec_address_hidden" name="rec_address" value="">
                                                                ...
                                                                @else
                                                                    <div class="alert alert-warning mb-0" role="alert">
                                                                        ...
                                                                        ท่านไม่มีสิทธิ์ในการเพิ่มสินทรัพย์ให้กับหน่วยใดๆ กรุณาติดต่อผู้ดูแลระบบ
                                                                    </div>
                                                                    <input type="hidden" name="rec_organize" value="">
                                                                    <input type="hidden" name="rec_address" value="">
                                                                @endif
                                                            </div>
                                                        </div>
                                                    </div>
...
<script>
    $(document).ready(function () {
        ...
        // auto-select ถ้ามีรายการเดียว
        var $dept = $('#department2');
        if ($dept.length) {
            var realOptions = $dept.find('option').filter(function () { return $(this).val() !== ''; });
            if (realOptions.length === 1) {
                $dept.val(realOptions.val()).trigger('change');
            }
        }
    });
</script>
