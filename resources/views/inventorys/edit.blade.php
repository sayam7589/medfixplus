...
@section('style')
    ...
    {{-- Select2 styling ใช้จาก medfix-theme.css (โทนธีมเดียวกันทุกหน้า) --}}
@endsection

@section('title', 'แก้ไขบัญชีสินทรัพย์')
        ...
        <div class="content-wrapper">
            ...
            <section class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1>แก้ไขบัญชีสินทรัพย์</h1>
                        </div>
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="/dashboard">หน้าหลัก</a></li>
                                <li class="breadcrumb-item active">แก้ไขบัญชีสินทรัพย์</li>
                            </ol>
                        </div>
                    </div>
                </div>
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <div class="card card-warning">
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    {{-- ============================================================
                                                         ORIGINAL CODE (เก็บไว้สำหรับ rollback - text + autocomplete)
                                                         ============================================================
                                                    <div class="row">
                                                        <div class="col-md-6">
                                                    ...
                                                    {{-- NEW: Dropdown (Select2) filtered by user roles --}}
                                                    <div class="row">
                                                        <div class="col-md-6">
                                                            <div class="form-group">
                                                                ...
                                                                @if($myDepartments->count() > 0)
                                                                    <select class="form-control select2-dept" id="department2" name="rec_organize" style="width: 100%;" required>
                                                                        <option value="">-- เลือกหน่วย/สังกัด --</option>
                                                                        @foreach($myDepartments as $dep)
                                                                            @php
                                                                                $parts = array_filter([$dep->short_name, $dep->gong, $dep->panag, $dep->fay], fn($v) => !empty(trim((string)$v)));
                                                                                $label = implode(' → ', $parts);
                                                                                $addressText = implode(' -> ', $parts);
                                                                            @endphp
                                                                            <option value="{{ $dep->id }}"
                                                                                data-address="{{ $addressText }}"
                                                                                {{ (string)$inventory->rec_organize === (string)$dep->id ? 'selected' : '' }}>
                                                                                {{ $label }}
                                                                            </option>
                                                                        @endforeach
                                                                    </select>
                                                                    <input type="hidden" id="rec_address_hidden" name="rec_address" value="{{ $inventory->rec_address }}">
                                                                    <small class="form-text text-muted mt-1">
                                                                        <i class="fas fa-info-circle"></i>
                                                                        แสดงเฉพาะหน่วยที่ท่านมีสิทธิ์ ({{ $myDepartments->count() }} รายการ)
                                                                    </small>
                                                                @else
                                                                    <div class="alert alert-warning mb-0" role="alert">
                                                                        <i class="fas fa-exclamation-triangle"></i>
                                                                        ท่านไม่มีสิทธิ์ในการแก้ไขหน่วย กรุณาติดต่อผู้ดูแลระบบ
                                                                    </div>
                                                                    <input type="hidden" name="rec_organize" value="{{ $inventory->rec_organize }}">
                                                                    <input type="hidden" name="rec_address" value="{{ $inventory->rec_address }}">
                                                                @endif
                                                            </div>
                                                        </div>
                                                    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // ปิดการใช้งานเมื่อเปลี่ยนเป็น dropdown แล้ว (เก็บไว้สำหรับ rollback)
        // setupAutocomplete('#department2', '#departmentList2', '#department_id2');
    });
</script>

{{-- Select2 init for หน่วย/สังกัด dropdown --}}
<script src="{{ asset('adminlte/plugins/select2/js/select2.full.min.js') }}"></script>
<script>
    $(document).ready(function () {
        $('.select2-dept').select2({
            theme: 'bootstrap4',
            placeholder: '-- เลือกหน่วย/สังกัด --',
            allowClear: true,
            language: {
                noResults: function () { return 'ไม่พบหน่วยที่ค้นหา'; },
        ...
        // sync rec_address (text ของหน่วยที่เลือก) ไปยัง hidden input
        $('#department2').on('change', function () {
            var selected = $(this).find(':selected');
            var addressText = selected.data('address') || '';
            $('#rec_address_hidden').val(addressText);
        });

        // trigger change ครั้งแรกเพื่อ sync rec_address กับค่าที่ preselected
        if ($('#department2').val()) {
            $('#department2').trigger('change');
        }
    });
</script>
<script>
