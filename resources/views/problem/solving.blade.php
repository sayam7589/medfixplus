                    ...
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach ($solvings as $solving)
                                <tr>
                                    <td>{{ $solving->id }}</td>
                                    <td>{{ $solving->solving_title }}</td>
                                    <td>
                                        <!-- Button trigger modal -->
                                        <button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#exampleModal-{{ $solving->id }}">
                                            แก้ไข
                                        </button>
                                        <a href="{{ route('problem_solving.destroy', $solving->id) }}" class="btn btn-danger btn-sm" data-confirm-delete="true">ลบ</a>
                                        <!-- Modal -->
                                        <form name="f{{ $solving->id }}" id="f{{ $solving->id }}" action="{{ route('problem_solving.update', $solving->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <div class="modal fade" id="exampleModal-{{ $solving->id }}" tabindex="-1" role="dialog"
                                                ...
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <input type="text" name="solving" id="" class="form-control" value="{{ $solving->solving_title }}">
                                                        </div>
